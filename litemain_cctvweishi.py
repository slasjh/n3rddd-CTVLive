                ...
                if category in channels:
                    for channel_name in channel_list:
                        if channel_name in channels[category]:
                            ...
                            #sorted_urls = sorted(unique_urls, key=lambda url: not is_ipv6(url) if lite_cctvweishi_con.ip_version_priority == "ipv6" else is_ipv6(url))
                            sorted_urls = unique_urls       
                            filtered_urls = [url for url in sorted_urls if not is_ipv6(url) and url not in written_urls and not any(blacklist in url for blacklist in lite_cctvweishi_con.url_blacklist)]
