...
def measure_speed(url):
    ...
    headers = {
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.6261.119 Safari/537.36 XiaoMi/MiuiBrowser/18.9.71225',
        #'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    ...
    def process_m3u8(m3u8_url):
        ...
        response = requests.get(m3u8_url, headers=headers, timeout=2)
        response.raise_for_status()
        lines = response.text.strip().split('\n')
        for line in lines:
            stripped_line = line.strip()
            if not stripped_line.startswith('#') and '.ts' in stripped_line:
                ts_url = url_t + stripped_line if not stripped_line.startswith('http') else stripped_line
                found = True
                break
        if not found:
            for line in lines:
